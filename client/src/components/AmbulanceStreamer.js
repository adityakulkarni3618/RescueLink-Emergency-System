...
export default function AmbulanceStreamer({ socket, connected }) {
    ...
    const vitalsInterval = setInterval(() => {
      ...
      if (socket && connected) {
        if (isOfflineRef.current) {
          // Cache locally during Dead Zone
  ...
  // GPS tracking loop - independent of streaming
  useEffect(() => {
    if (navigator.geolocation) {
  ...
  }, [socket, connected, selectedHospital]);
  ...
  const sendHospitalRequest = (directPatientId = null, isForce = false, directDetails = null, isPhase2 = false) => {
    ...
    // Background: try to find a default hospital for GPS navigation
    const hList = Object.keys(networkHospitals).length > 0 ? Object.entries(networkHospitals).map(([id, h]) => ({ id, ...h })) : HOSPITALS;
    if (hList.length > 0) {
      const firstHosp = hList[0];
      setSelectedHospital({ ...firstHosp, pos: firstHosp.location || firstHosp.pos, baseDistance: 15 });
    }
  };
  ...
  const acceptRequest = () => {
    ...
    // PHASE 1: Broadcast Lite Alert (No report/vitals yet)
      ...
      <div style={{
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 20, alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 12 }}>
            ...
            >
              {isOffline ? '⚠️ DEAD ZONE ACTIVE' : (signalLostTime > 0 ? `🆘 SIGNAL LOST (${signalLostTime}s)` : '📶 DEAD ZONE')}
            </button>
            <button
