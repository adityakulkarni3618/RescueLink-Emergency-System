...
export default function AmbulanceMarketplace({ socket, userLocation, onBookAmbulance }) {
  ...
  const filtered = Array.isArray(ambulances) ? ambulances
    .filter(a => selectedType === 'all' || (a.type && a.type.toLowerCase().includes(selectedType.toLowerCase())))
    .sort((a, b) => {
              ...
              return (
                <div key={amb.id} onClick={() => setSelected(amb)} style={{
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
                    {Array.isArray(amb.features) && amb.features.map((f, fi) => (
                      <span key={fi} style={{
