...
/**
 * @route GET /api/hospitals/purge-demo-entities
 * @desc Purge historical demo/seeded entities from database
 */
router.get('/purge-demo-entities', async (req, res) => {
  try {
    ...
    const remainingHospitals = await Hospital.findAll({ order: [['createdAt', 'DESC']] });
    const remainingAmbulances = await Ambulance.findAll({ order: [['createdAt', 'DESC']] });

    return res.json({
      success: true,
      message: `Purged ${delH} demo hospitals and ${delA} demo ambulances.`,
      remainingHospitals,
      remainingAmbulances
    });
  } catch (err) {
    console.error('[PURGE ROUTE ERROR]', err.message);
