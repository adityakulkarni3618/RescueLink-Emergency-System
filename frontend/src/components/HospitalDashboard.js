...
function AbdmConnectModal({ patient, onClose, onLinked }) {
  ...
  const requestOtp = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`${API_BASE_URL}/api/auth/lookup-abha/${abhaId}`);
      const data = await res.json();
      if (res.ok) {
        setStep(2);
      } else {
...
export default function HospitalDashboard({ socket, connected, onLogout, onSwitchRole, onShowSecurity }) {
  ...
  // Fetch real database profile coordinates for authenticated hospital tenant
  useEffect(() => {
    const hydratetHospitalCoords = async () => {
      const hid = authHospital?.hospital_id || authHospital?.hospitalId || authHospital?.id;
      if (!hid || hid === 'hosp_demo_center_1') return;
      try {
        ...
        if (res.ok) {
          ...
          if (data && data.lat && data.lng && !isNaN(parseFloat(data.lat)) && !isNaN(parseFloat(data.lng))) {
            ...
            const trueLng = parseFloat(data.lng);
            setHospitalGps({ lat: trueLat, lng: trueLng });
        ...
        console.warn('[HOSPITAL DB HYDRATE] Failed to fetch hospital coordinates:', err);
      }
    };
    hydratetHospitalCoords();
  }, [authHospital?.hospital_id, authHospital?.hospitalId, authHospital?.id]);
    ...
    try {
      ...
      if (res.ok && data.token) {
        ...
        const found = {
          ...
          name: data.user?.hospitalName || data.user?.name || 'City Emergency Command Center',
          adminName: data.user?.name || 'Hospital Coordinator',
          internalId: (data.user?.hospital_id || inputId).toLowerCase(),
          lat: data.user?.lat || data.lat || 19.0760,
          lng: data.user?.lng || data.lng || 72.8777
        };
