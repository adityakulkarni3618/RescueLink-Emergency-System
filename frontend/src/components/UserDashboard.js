    ...
    <div style={{ height: '100vh', display: 'flex', flexDirection: 'column', background: '#050a1e', color: '#e0eaff', fontFamily: "'Rajdhani', sans-serif" }}>
      ...
      <div className="main-content-layout" style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <div className="map-view-container" style={{ flex: 1, position: 'relative' }}>
          ...
          <MapContainer
            center={(mapCenter && typeof mapCenter[0] === 'number' && !isNaN(mapCenter[0])) ? mapCenter : [12.9716, 77.5946]}
            zoom={14}
          ...
          >
            ...
            {routePath && Array.isArray(routePath) && (
              <>
                <Polyline positions={routePath.filter(isValidLatLng)} color="#00ff88" weight={5} opacity={0.7} dashArray="10, 10" />
                {routePath.length > 2 && [routePath[Math.floor(routePath.length / 3)], routePath[Math.floor(routePath.length * 2 / 3)]].filter(isValidLatLng).map((pos, idx) => {
